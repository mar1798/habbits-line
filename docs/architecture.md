...
- **Списки** — `FlatList`, не `ScrollView` + `.map`. Исключение — короткий список
  фиксированной длины внутри уже скроллящегося экрана (доходы периода в модалке
  бюджета): `VirtualizedList` внутри `ScrollView` той же ориентации даёт
  предупреждение RN, и виртуализация на горстке строк ничего не стоит.
