...
/** One offer of the quick-add row: a category and an amount spent together before. */
export interface QuickEntry {
  categoryId: string;
  amount: number;
  /** The description of the latest expense with this category and amount, if it had one. */
  note: string | null;
  /** How many times the pair occurs in the history it was derived from. */
  count: number;
}

/**
 * What the quick-add row above the day list offers: distinct (category, amount) pairs out
 * of recent history, at most `limit` of them.
 *
 * The newest pair is kept first whatever its count — that slot is "repeat the last
 * expense", and a one-off coffee bought an hour ago has to be there even though every
 * other pair is more frequent. The rest are ranked by how often they repeat, ties broken
 * by recency, which is what makes the row a set of templates rather than a log.
 *
 * The description is not part of the key: two "Food, 200" with different notes would show
 * as two identical chips, so a pair takes the note of its latest occurrence and the chip
 * writes that. `expenses` must be ordered newest first — the caller's query already is,
 * and that order is what both "the newest pair" and the tie-break read.
 */
export function quickEntries(expenses: QuickEntryItem[], limit: number): QuickEntry[] {
  if (limit <= 0) return [];

  const pairs = new Map<string, QuickEntry & { rank: number }>();
  expenses.forEach((expense, index) => {
    // Serialized rather than joined through a separator: a category id is opaque text,
    // and an imported one containing that separator would merge two distinct pairs.
    const key = JSON.stringify([expense.category_id, expense.amount]);
  ...
  // Insertion order is recency order, so the head is the newest pair by construction.
  const [newest, ...rest] = Array.from(pairs.values());
  if (!newest) return [];

  rest.sort((a, b) => b.count - a.count || a.rank - b.rank);

  return [newest, ...rest]
    .slice(0, limit)
    .map(({ categoryId, amount, note, count }) => ({ categoryId, amount, note, count }));
}
